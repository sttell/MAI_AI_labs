...
class HimmelblauOptimizationProblem:
...
udp = HimmelblauOptimizationProblem(-5.0, 5.0, -5.0, 5.0)
...
ax.scatter(champion_n, np.min(fitness_list), marker='x', color='r', label='Best champion')

# Оформление
ax.set_xlim((0, number_of_evolutions))
ax.grid('major')
ax.set_title('The best representative of each epoch', fontweight='bold')
...
plt.savefig('best_himmelblau.png')
plt.clf()
...
plt.savefig('himmelblau_result.png')
plt.clf()
